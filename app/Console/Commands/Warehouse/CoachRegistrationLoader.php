...
class CoachRegistrationLoader implements LoaderInterface
{
    protected $maxStageId = 0;
        
    /**
     * Load staged Coach Registration data/records into Coach Registration cube
     *
    ...
    protected function findMaxRecordId()
    {
        ...
        $maxIdResult = DB::connection('mysql-dw')->selectOne('
            SELECT MAX(id) AS max_id FROM stg_coach_reg_events
        ');
    ...
    protected function prepareRecords()
    {
        DB::connection('mysql-dw')->statement('
            DROP TABLE IF EXISTS `wrk_stg_coach_reg_events`
        ');
        DB::connection('mysql-dw')->statement(
            'CREATE TABLE wrk_stg_coach_reg_events ENGINE=Innodb CHARSET=utf8 AS 
            SELECT * FROM stg_coach_reg_events WHERE id <= ?',
            [ $this->maxStageId ]
        );
        $indexes = [
            'created_at',
        ...
        foreach ($indexes as $index) {
            DB::connection('mysql-dw')->statement("
                CREATE INDEX idx_${index} ON wrk_stg_coach_reg_events (${index})
            ");
        }
        return true;
    }
    ...
    protected function dedupeRecords()
    {
        ...
        DB::connection('mysql-dw')->statement('
            DROP TABLE IF EXISTS `wrk_stg_coach_reg_events_unique`
        ');
        DB::connection('mysql-dw')->statement('
            CREATE TABLE wrk_stg_coach_reg_events_unique ENGINE=Innodb CHARSET=utf8 AS
            SELECT
                w.*
            FROM wrk_stg_coach_reg_events w
            JOIN (
                SELECT
                    registration_id AS registration_id,
                    MAX(created_at) AS created_at
                FROM
                    wrk_stg_coach_reg_events
                GROUP BY
                    registration_id                
            ) max_w ON 
    ...
    protected function validateRecords()
    {
        ...
        DB::connection('mysql-dw')->statement('
            DROP TABLE IF EXISTS `wrk_stg_coach_reg_events_valid`
        ');

        DB::connection('mysql-dw')->statement('
            CREATE TABLE wrk_stg_coach_reg_events_valid ENGINE=Innodb CHARSET=utf8 AS
            SELECT
                event_id,
                event_created_at,
                registration_id,
            ...
            FROM (
                SELECT
                    ...
                    dim_coaches.id AS dim_coach_id,
                    ...
                    IF(dim_coaches.id IS NULL, 1, 0) AS invalid_2,
                    IF(dim_dates.id IS NULL, 1, 0) AS invalid_3
                FROM wrk_stg_coach_reg_events_unique w
                LEFT JOIN dim_coaches ON 
                    dim_coaches.coach_id = w.coach_id
                ...
                LEFT JOIN dim_positions ON
                    (dim_positions.position_type = "coaches" AND dim_positions.position = w.position)
            ) AS validation
        ');
        
        DB::connection('mysql-dw')->statement('
            CREATE INDEX idx_invalid ON wrk_stg_coach_reg_events_valid (invalid)
        ');
    ...
    protected function addRecords()
    {
        ...
        DB::connection('mysql-dw')->statement('
            INSERT INTO cube_coach_registrations (
                registration_id,
                dim_date_id,
                dim_coach_id,
                dim_level_id,
                ...
                NOW()
            FROM
                wrk_stg_coach_reg_events_valid AS w
            WHERE
                w.invalid = 0
    ...
    protected function removeStagedRecords()
    {
        ...
        DB::connection('mysql-dw')->statement(
            'DELETE FROM stg_coach_reg_events WHERE id <= ?',
            [ $this->maxStageId ]
        );
        return true;
    }
